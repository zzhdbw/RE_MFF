...
'''最终向量,提取实体位置向量作为分类器的输入 使用节点距离加权邻接矩阵'''
class Model(nn.Module):
    ...
    def forward(self, input_ids, attention_masks, segment_ids, entity_token_ids=None, 
        ...
        dep_adj_matrix = dep_distence_matrix
        if self.renorm != 0: ##使用重整化参数
            diag = torch.full(size=[batch_size,max_word_len], fill_value=self.renorm-1, device=dep_type_matrix.device)   ## 邻接矩阵本就带着自身节点的链接，所以要先减去1
