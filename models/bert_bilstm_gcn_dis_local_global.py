...
class Model(nn.Module):
    ...
    def __init__(self, args, config):
        ...
        self.gcn_layer_local1 = GraphConvolution(args.grnn_dim*2, args.grnn_dim*2,self.renorm)
        self.gcn_layer_local2 = GraphConvolution(args.grnn_dim*2, args.grnn_dim*2,self.renorm)
        ...
        self.classifier = nn.Linear(args.grnn_dim * 2 * 6 + config.hidden_size * 5, self.num_labels)
    ...
    def forward(self, input_ids, attention_masks, segment_ids, entity_token_ids=None, 
        ...
        ## dep_type_matrix以数组形式存储 dis和 local
        dep_type_matrix_dis = dep_type_matrix[:,0,:,:]   ## 距离矩阵获取的依存类型图为全图
        dep_type_matrix_local = dep_type_matrix[:,1,:,:]   
        dep_type_matrix_global = dep_type_matrix[:,2,:,:]   
        dep_adj_matrix_local = torch.clamp(dep_type_matrix_local, min=0, max=1)
        dep_adj_matrix_global = torch.clamp(dep_type_matrix_global, min=0, max=1)

        word_out_local = self.gcn_layer_local1(word_out, dep_adj_matrix_local)
        word_out_local = self.gcn_layer_local2(word_out_local, dep_adj_matrix_local)    ## [1, 80, 256]  ## e1_mask [1, 80]
        e1_h_local = self.extract_entity(word_out_local, e1_mask)       ## [1, 256]
        e2_h_local = self.extract_entity(word_out_local, e2_mask)
        ...
        out = torch.cat([e1_h_local,e2_h_local,e1_h_global,e2_h_global,e1_h_dis,e2_h_dis,bert_out],dim=-1)  ## 4864
